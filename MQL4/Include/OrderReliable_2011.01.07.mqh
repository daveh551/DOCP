					  ...
					  color arrow_color = CLR_NONE) 
{
   static bool bridge=false;
	...
	OrderReliablePrint("attempted " + OrderReliable_CommandString(cmd) + " " + DoubleToString(volume) + 
						" lots @" + DoubleToStr(price, _Digits) + " sl:" + DoubleToStr(stoploss, _Digits) + 
	...
	if ((cmd == OP_BUYSTOP) || (cmd == OP_SELLSTOP) || (cmd == OP_BUYLIMIT) || (cmd == OP_SELLLIMIT)) 
	{
		...
		if (!limit_to_market) 
		{
			...
			OrderReliablePrint("failed trade: " + OrderReliable_CommandString(cmd) + " " + symbol + 
								"@" + DoubleToStr(price, _Digits) + " tp@" + DoubleToStr(takeprofit, _Digits) +
								 " sl@" + DoubleToStr(stoploss, _Digits)); 
...
//            	Print("OrderModifyReliable ticket = ",OrderTicket(),", price = ",OrderOpenPrice(),", stoploss = ",OrderStopLoss(),", takeprofit = ",OrderTakeProfit());
			...
			RefreshRates();
				OrderReliablePrint("retryable error (" + IntegerToString(cnt) + "/" + 
									IntegerToString(retry_attempts) + "): " + OrderReliableErrTxt(err)+ " (price = "+ DoubleToStr(price, _Digits)
									 + ", Ask = "+ DoubleToStr(Ask, _Digits) +", and Bid = "+ DoubleToStr(Bid, _Digits) +")"); 
				OrderReliable_SleepRandomTime(sleep_time,sleep_maximum); 
				RefreshRates();
      		if (cmd == OP_BUY)
		         {
...
*/	
	...
	if (err == ERR_NO_RESULT) 
	{
		...
		OrderReliablePrint("redundant modification: "  + ticket + " " + _Symbol + 
							"@" + DoubleToStr(price, _Digits) + " tp@" + DoubleToStr(takeprofit, _Digits) + 
							" sl@" + DoubleToStr(stoploss,_Digits)); 
	...
	OrderReliablePrint("failed modification: "  + ticket + " " + _Symbol + 
						"@" + DoubleToStr(price, _Digits) + " tp@" + DoubleToStr(takeprofit, _Digits) + 
						" sl@" + DoubleToStr(stoploss, _Digits)); 
	OrderReliablePrint("last error: " + OrderReliableErrTxt(err)); 
...
*/	
	...
	while (!exit_loop) 
	{
		if (IsTradeAllowed()) 
		{
   		if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)==true)
            ...
            {
               err = GetLastError();
               OrderReliablePrint("error: unable to select Order for close: error = " + IntegerToString(err));
            }
		} 
		else 
			cnt++;
	...
	OrderReliablePrint("last error: " + OrderReliableErrTxt(err)+ " (price = "+ 
	   DoubleToStr(price, _Digits) + ", Ask = "+ DoubleToStr(Ask, _Digits) +", and Bid = "+ DoubleToStr(Bid, _Digits) +")"); 
					  ...
					  color arrow_color = CLR_NONE) 
{
	...
	OrderReliablePrint(" attempted " + OrderReliable_CommandString(cmd) + " " + DoubleToStr(volume,0) + 
						" lots @" + DoubleToStr(price, _Digits) + " sl:" + DoubleToStr(stoploss, _Digits) + 
						" tp:" + DoubleToStr(takeprofit, _Digits)); 
...
*/	
	...
	if ((cmd == OP_BUYSTOP) || (cmd == OP_SELLSTOP) || (cmd == OP_BUYLIMIT) || (cmd == OP_SELLLIMIT)) 
	{
		...
		if (!limit_to_market) 
		{
			OrderReliablePrint("failed to execute stop or limit order after " + IntegerToString(cnt) + " retries");
			OrderReliablePrint("failed trade: " + OrderReliable_CommandString(cmd) + " " + symbol + 
								"@" + DoubleToStr(price, _Digits) + " tp@" + DoubleToStr(takeprofit, _Digits) + " sl@" + DoubleToStr(stoploss, _Digits)); 
			OrderReliablePrint("last error: " + OrderReliableErrTxt(err)); 
			return(-1); 
		}
	}  // end	  
	...
	if ((cmd == OP_BUY) || (cmd == OP_SELL)) 
	{
		...
		while (!exit_loop) 
		{
			...
			RefreshRates();
				OrderReliablePrint("retryable error (" + IntegerToString(cnt) + "/" + 
									IntegerToString(retry_attempts) + "): " + OrderReliableErrTxt(err)+ " (price = "+ 
									DoubleToStr(price, _Digits) + ", Ask = "+ DoubleToStr(Ask, _Digits) +", and Bid = "+ DoubleToStr(Bid, _Digits) +")"); 
				OrderReliable_SleepRandomTime(sleep_time,sleep_maximum); 
				RefreshRates();
      		if (cmd == OP_BUY)
